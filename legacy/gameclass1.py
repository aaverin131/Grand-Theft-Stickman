...
class World:#class for creating a game window and other related stuff
    ...
    def rotation(self,item):#saved for later
        pass
    def matrix(self,matrix,game,objects,scale):#                                                                                                DELETEEEEEEEEEEEEEEE
        y = 0
        for row in matrix:
            x=0
            for item in row:
                ...
                if item == 2:
                    ...
                    game.append(objects[1][0],(x * 16- scale,y * 16-scale))
                elif item == 3:
                    pass
                elif item == 3:
                    pass
    ...
    '''def load(self, folder,name,size =""):
        self.image = pygame.image.load(os.path.join(folder, name))
        if isinstance(size, tuple)==True:
            self.image = pygame.transform.scale(self.image,size)
        else:
...
class Movement(Image):
    ...
    def action(self,slotnum,inventory,game,s_width,s_height,angle,stickmanhead,glockhold):
        ...
        try:#prevents the errors
            ...
            '''orig_rect = inventory[slotnum][1].get_rect()
            rotated_image = pygame.transform.rotate(inventory[slotnum][1], angle)
            rotated_rect = orig_rect.copy()
            rotated_rect.center = rotated_image.get_rect().center
            rotated_image = rotated_image.subsurface(rotated_rect).copy()
            inventory[slotnum][1] = rotated_image'''
...
#ave = FileSave()
#writing = save.writecsv('stickman.png',123,124,500,100,'glock',1,1,1,1,1,1,'bank')
#read = save.readcsv()
#numsave = save.picksave(2)
#print(numsave)
#saves.read()
#print(saves.saves)
#print()
#saves.write('stickmanweapon.png',[['punch'],['glock'],['ak47'],['1'],['1']],['900','200'],'900','90')
'''with open("new.txt","w") as new:
    new.write('stickman.png, punch, glock, ak47, 1, 1, 900, 200, 900, 90')
new.close()'''
    
#print(saves.saves)

#print(saves.picksave(1))
'''stickman,inventory,xy,money,health
'stickman.png',[[punch],[glock],[1],[1],[1]],[300,300],500,100'''
'''class GameMap(World):
    def __init__(self):
        super().__init__
    def gamemap(self,condition,game):
        if condition == 'world':
            game.fill((120,214,60))
        ...
        if condition == 'bank':
            ...
            gamemap.show('gunshop')
        elif condition == 'house':
            gamemap.show('house')
        elif condition == 'basement':
            gamemap.show('basement')'''

'''
class Events_info(Stickman):
    def __init__(self):
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                run = False #SET FALSE TO QUIT
                pygame.quit()
            if event.type == pygame.MOUSEMOTION:
                # get the mouse position as a tuple of (x, y) coordinates
                self.mouse_pos = pygame.mouse.get_pos()
                print("Mouse position:", self.mouse_pos)
            ...
            if event.type == pygame.MOUSEWHEEL:
                ...
                self.eventy =event.y
            if True in pygame.mouse.get_pressed():
                    # get the mouse button state as a tuple of (left, middle, right) booleans
                    self.mouse_buttons = pygame.mouse.get_pressed()
                    print("Mouse buttons:", self.mouse_buttons)'''